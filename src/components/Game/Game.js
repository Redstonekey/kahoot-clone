...
class Game extends Component {
    ...
    generatePin() {
        let newPin = Math.floor(Math.random() * 9000) + 1000;
...
timeCheck() {
    ...
    timeKeeper() {
        ...
        let timeKept = setInterval(this.timeCheck, 1000);
        return timeKept;
    }
    ...
    submitAnswer(name, answer) {
        ...
        if (player) {
            player.qAnswered = true;
            player.answeredCorrect = answer === this.state.questions[this.state.currentQuestion].correctanswer;
